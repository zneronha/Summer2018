...
for condnum = 1:numel(AssayOHT)
    ...
    [row, col] = find(storeNeighbors(:,1:(4*hrscan))==0);
    ...
    %row and column where first individual CB recorded
    Ax3 = [IndList IndCol];
    %now find where neighbors are found
    [row1, col1] = find(storeNeighbors(:,1:(4*hrscan))>0);
        % pull out unique cell IDs
    IndList1 = unique(row1); %all the rows that are individuals at an early timepoint
    
    IndCol1 = [];
    
    %pull out first occurance of multi cell behavior:
    for j = IndList1'
       ix31 = row1 == j;
       relCol1 = col1(ix31);
       sp11 = min(relCol1);
    ...
    %Reset IndList from the excised cells
